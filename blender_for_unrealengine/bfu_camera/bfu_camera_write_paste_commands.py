...
def add_camera_to_command(camera: bpy.types.Object, pre_bake_camera: bfu_camera_data.BFU_CameraTracks):
...
def get_import_camera_script_command(objs: List[bpy.types.Object]) ->  Tuple[bool, str, str]:
    ...
    for camera in cameras:
        add_command = add_camera_to_command(camera, pre_bake_camera.get_evaluate_camera_data(camera))
        if add_command:
            t += add_command
            add_camera_num += 1
